# ============================================================================
# LEfSe Differential KEGG Boxplot with Statistical Annotation (HQQD vs Model Selection)
# Author: Generated for metagenomics analysis
# Date: 2026-02-24
# Description: Horizontal boxplot showing differential KEGG functions
...
abundance_file <- "kegg.kegg_pathway_level3.relabundance.xlsx"
output_dir <- "07b_HQQD_reverse_KEGG"
...
kegg_levels <- list(
  list(name = "Level3", column = "kegg_pathway")
)
...
cat("KEGG data loaded:", nrow(abundance_data), "features\n")
...
match_feature_to_abundance <- function(feature_string, abundance_data) {
  ...
  for (level in levels) {
    if (grepl("^L1__", level)) {
      conditions <- conditions & (abundance_data[["KEGG_pathway level 1"]] == sub("^L1__", "", level))
    } else if (grepl("^L2__", level)) {
      conditions <- conditions & (abundance_data[["KEGG_pathway level 2"]] == sub("^L2__", "", level))
    } else if (grepl("^L3__", level)) {
      conditions <- conditions & (abundance_data[["kegg_pathway"]] == sub("^L3__", "", level))
    }
  }
...
get_lefse_lda_for_feature <- function(feature_string, abundance_data, group1_name, group2_name, kegg_level, sample_metadata, sample_cols) {
  cache_key <- paste(kegg_level, group1_name, group2_name, sep = "_")
  ...
  if (!(cache_key %in% names(LEFSE_CACHE))) {
    ...
    level_col <- switch(kegg_level,
      "Level1" = "KEGG_pathway level 1",
      "Level2" = "KEGG_pathway level 2",
      "Level3" = "kegg_pathway"
    )
    ...
    agg_abundance$Feature_ID <- paste0("KEGG_", sprintf("%05d", seq_len(nrow(agg_abundance))))
    ...
    tax_table_comp <- data.frame(
      Kingdom = rep(kegg_level, nrow(agg_abundance)),
      Phylum = rep(kegg_level, nrow(agg_abundance)),
    ...
    tryCatch({
      ...
      if (!is.null(comp_lefse$res_diff) && nrow(comp_lefse$res_diff) > 0) {
        comp_lefse$res_diff$Taxon_Name <- sapply(comp_lefse$res_diff$Taxa, function(x) {
          if (grepl("^KEGG_", x) && x %in% names(feature_names_map)) return(feature_names_map[[x]])
          parts <- strsplit(x, "\\|")[[1]]
  ...
  target_feature <- sub("^L[123]__", "", get_clean_name(feature_string, keep_prefix = TRUE))
  matching_rows <- lefse_res[lefse_res$Taxon_Name == target_feature, ]
  if (nrow(matching_rows) > 0) return(matching_rows$LDA[1])
  0
}

check_callback_trend <- function(feature_string, abundance_data, kegg_level, sample_metadata, sample_cols) {
  ...
  lda_model_control <- get_lefse_lda_for_feature(feature_string, abundance_data, "Model", "Control", kegg_level, sample_metadata, sample_cols)
  lda_hqqd_model <- get_lefse_lda_for_feature(feature_string, abundance_data, "HQQD", "Model", kegg_level, sample_metadata, sample_cols)
...
save_reverse_boxplot <- function(plot_data_combined, output_file_pdf, output_file_png, plot_title,
  ...
  plot_height <- max(6, n_features * 0.55 + 2)
...
for (level_info in kegg_levels) {
  kegg_level <- level_info$name
  level_column <- level_info$column

  cat("\n", rep("=", 70), "\n", sep = "")
  cat("=== Processing KEGG", kegg_level, "===\n")
  cat(rep("=", 70), "\n\n", sep = "")
  ...
  cat("Total KEGG features at", kegg_level, ":", length(unique_features), "\n")

  level_results <- data.frame(Taxa = character(), stringsAsFactors = FALSE)

  for (feature in unique_features) {
    repr_row <- abundance_data %>% filter(.data[[level_column]] == feature) %>% slice(1)

    feature_string <- if (kegg_level == "Level1") {
      paste0("L1__", feature)
    } else if (kegg_level == "Level2") {
      paste0("L1__", repr_row[["KEGG_pathway level 1"]], "|", "L2__", repr_row[["KEGG_pathway level 2"]])
    } else {
      paste0("L1__", repr_row[["KEGG_pathway level 1"]], "|", "L2__", repr_row[["KEGG_pathway level 2"]], "|", "L3__", repr_row[["kegg_pathway"]])
    }
  ...
  for (i in seq_len(nrow(level_results))) {
    callback_results[[i]] <- check_callback_trend(level_results$Taxa[i], abundance_data, kegg_level, sample_metadata, sample_cols)
    if (i %% 10 == 0) cat(".")
  }
  cat("\n")
  ...
  cat("KEGG features with significant callback trend:", nrow(sig_results), "\n")
  cat("KEGG features retained after BH correction:", sum(level_results$is_callback_fdr, na.rm = TRUE), "\n")
  ...
  p_left <- ggplot(plot_data_combined, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
    ...
    labs(
      x = expression(Log[10] ~ "Relative Abundance"),
      y = NULL,
      title = paste0(nrow(top_features), " Differential KEGG Features at ", kegg_level),
  ...
  output_file_pdf <- file.path(output_dir, paste0("LEfSe_boxplot_KEGG_", kegg_level, ".pdf"))
  output_file_png <- file.path(output_dir, paste0("LEfSe_boxplot_KEGG_", kegg_level, ".png"))

  plot_height <- max(6, nrow(top_features) * 0.55 + 2)
  plot_width <- ifelse(kegg_level == "Level3", 8.5, 8.5)
  ...
  level3_meta <- abundance_data %>%
    select(kegg_pathway, kegg_pathway_id, `KEGG_pathway level 2`, `KEGG_pathway level 1`) %>%
    distinct(kegg_pathway, .keep_all = TRUE)

  stat_summary <- stat_summary %>%
    left_join(level3_meta, by = c("Taxon" = "kegg_pathway"))

  final_cols <- c(
    "Taxon", "kegg_pathway_id", "KEGG_pathway level 2", "KEGG_pathway level 1", "Taxa_Full",
  ...
  write.csv(stat_summary,
    file.path(output_dir, paste0("statistical_results_KEGG_", kegg_level, ".csv")),
    row.names = FALSE
  )

  write.csv(stat_summary_fdr,
    file.path(output_dir, paste0("statistical_results_KEGG_", kegg_level, "_fdr.csv")),
    row.names = FALSE
  )
  ...
  if (nrow(stat_summary_fdr) > 0) {
    ...
    save_reverse_boxplot(
      plot_data_fdr,
      file.path(output_dir, paste0("LEfSe_boxplot_KEGG_", kegg_level, "_fdr.pdf")),
      file.path(output_dir, paste0("LEfSe_boxplot_KEGG_", kegg_level, "_fdr.png")),
      "KEGG pathways",
      plot_width = 9
    )
  }
...
cat("\n", rep("=", 70), "\n", sep = "")
cat("All KEGG boxplots generated successfully!\n")
cat("Output directory:", output_dir, "\n")
cat("\nGenerated files:\n")
cat("  - LEfSe_boxplot_KEGG_Level3.pdf/png\n")
cat("  - LEfSe_boxplot_KEGG_Level3_fdr.pdf/png\n")
cat("  - statistical_results_KEGG_Level3.csv (Wilcoxon p-values: Model vs Control, HQQD vs Model, Model vs LVX)\n")
cat("  - statistical_results_KEGG_Level3_fdr.csv (BH-corrected callback subset)\n")
cat(rep("=", 70), "\n\n", sep = "")
