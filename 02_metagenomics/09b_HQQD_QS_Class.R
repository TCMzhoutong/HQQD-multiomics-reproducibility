# ============================================================================
# LEfSe Differential QS_Class Boxplot with Statistical Annotation (HQQD vs Model Selection)
# Author: Generated for metagenomics analysis
# Date: 2026-02-24
# Description: Horizontal boxplot showing differential QS_Class functions
...
abundance_file <- "QS.class.relabundance.xlsx"
output_dir <- "09b_HQQD_QS_Class"
...
qs_levels <- list(
  list(name = "Class", column = "Quorum_Sensing_Class")
)
...
cat("QS data loaded:", nrow(abundance_data), "features\n")
...
match_feature_to_abundance <- function(feature_string, abundance_data) {
  target_feature <- sub("^QS__", "", get_clean_name(feature_string, keep_prefix = TRUE))
  matched_rows <- abundance_data[abundance_data[["Quorum_Sensing_Class"]] == target_feature, ]
...
get_lefse_lda_for_feature <- function(feature_string, abundance_data, group1_name, group2_name, qs_level, sample_metadata, sample_cols) {
  cache_key <- paste(qs_level, group1_name, group2_name, sep = "_")
  ...
  if (!(cache_key %in% names(LEFSE_CACHE))) {
    ...
    agg_abundance <- abundance_data %>%
      filter(!is.na(Quorum_Sensing_Class) & Quorum_Sensing_Class != "") %>%
      group_by(Quorum_Sensing_Class) %>%
    ...
    agg_abundance$Feature_ID <- paste0("QS_", sprintf("%05d", seq_len(nrow(agg_abundance))))
    ...
    tax_table_comp <- data.frame(
      Kingdom = rep(qs_level, nrow(agg_abundance)),
      Phylum = rep(qs_level, nrow(agg_abundance)),
    ...
    tryCatch({
      ...
      if (!is.null(comp_lefse$res_diff) && nrow(comp_lefse$res_diff) > 0) {
        comp_lefse$res_diff$Taxon_Name <- sapply(comp_lefse$res_diff$Taxa, function(x) {
          if (grepl("^QS_", x) && x %in% names(feature_names_map)) return(feature_names_map[[x]])
          parts <- strsplit(x, "\\|")[[1]]
...
check_callback_trend <- function(feature_string, abundance_data, qs_level, sample_metadata, sample_cols) {
  ...
  lda_model_control <- get_lefse_lda_for_feature(feature_string, abundance_data, "Model", "Control", qs_level, sample_metadata, sample_cols)
  lda_hqqd_model <- get_lefse_lda_for_feature(feature_string, abundance_data, "HQQD", "Model", qs_level, sample_metadata, sample_cols)
...
for (level_info in qs_levels) {
  qs_level <- level_info$name
  level_column <- level_info$column

  cat("\n", rep("=", 70), "\n", sep = "")
  cat("=== Processing QS", qs_level, "===\n")
  cat(rep("=", 70), "\n\n", sep = "")
  ...
  cat("Total QS features at", qs_level, ":", length(unique_features), "\n")

  level_results <- data.frame(Taxa = character(), stringsAsFactors = FALSE)

  for (feature in unique_features) {
    feature_string <- paste0("QS__", feature)
  ...
  for (i in seq_len(nrow(level_results))) {
    callback_results[[i]] <- check_callback_trend(level_results$Taxa[i], abundance_data, qs_level, sample_metadata, sample_cols)
    if (i %% 10 == 0) cat(".")
  }
  cat("\n")
  ...
  cat("QS features with significant callback trend:", nrow(sig_results), "\n")
  cat("QS features retained after BH correction:", sum(level_results$is_callback_fdr, na.rm = TRUE), "\n")
  ...
  p_left <- ggplot(plot_data_combined, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
    ...
    labs(
      ...
      y = NULL,
      title = paste0(nrow(top_features), " Differential QS Features at ", qs_level),
  ...
  output_file_pdf <- file.path(output_dir, paste0("LEfSe_boxplot_QS_", qs_level, ".pdf"))
  output_file_png <- file.path(output_dir, paste0("LEfSe_boxplot_QS_", qs_level, ".png"))
  ...
  qs_meta <- abundance_data %>%
    select(Quorum_Sensing_Class) %>%
    distinct(Quorum_Sensing_Class, .keep_all = TRUE)

  stat_summary <- stat_summary %>%
    left_join(qs_meta, by = c("Taxon" = "Quorum_Sensing_Class"))
  ...
  write.csv(stat_summary,
    file.path(output_dir, paste0("statistical_results_QS_", qs_level, ".csv")),
    row.names = FALSE
  )

  write.csv(stat_summary_fdr,
    file.path(output_dir, paste0("statistical_results_QS_", qs_level, "_fdr.csv")),
    row.names = FALSE
  )
  ...
  if (nrow(stat_summary_fdr) > 0) {
    ...
    save_reverse_boxplot(
      plot_data_fdr,
      file.path(output_dir, paste0("LEfSe_boxplot_QS_", qs_level, "_fdr.pdf")),
      file.path(output_dir, paste0("LEfSe_boxplot_QS_", qs_level, "_fdr.png")),
      paste0(nrow(stat_summary_fdr), " FDR-corrected Differential QS Features at ", qs_level),
...
cat("\n", rep("=", 70), "\n", sep = "")
cat("All QS boxplots generated successfully!\n")
cat("Output directory:", output_dir, "\n")
cat("\nGenerated files:\n")
cat("  - LEfSe_boxplot_QS_Class.pdf/png\n")
cat("  - LEfSe_boxplot_QS_Class_fdr.pdf/png\n")
cat("  - statistical_results_QS_Class.csv (Wilcoxon p-values: Model vs Control, HQQD vs Model, Model vs LVX)\n")
cat("  - statistical_results_QS_Class_fdr.csv (BH-corrected callback subset)\n")
cat(rep("=", 70), "\n\n", sep = "")
