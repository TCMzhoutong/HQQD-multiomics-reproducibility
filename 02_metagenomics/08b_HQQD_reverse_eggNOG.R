# ============================================================================
# LEfSe Differential eggNOG Boxplot with Statistical Annotation (HQQD vs Model Selection)
# Author: Generated for metagenomics analysis
# Date: 2026-02-24
# Description: Horizontal boxplot showing differential eggNOG functions
...
abundance_file <- "eggNOG.NOG.relabundance.xlsx"
output_dir <- "08b_HQQD_reverse_eggNOG"
...
cat("eggNOG data loaded:", nrow(abundance_data), "features\n")

# Use current COG definitions for the publication-facing descriptions.
# The original input contains abbreviated labels and a typo (COG0745: "regulatoR").
...
get_clean_name <- function(feature_string, keep_prefix = FALSE) {
  ...
  sub("^NOG__", "", last_level)
}

match_feature_to_abundance <- function(feature_string, abundance_data) {
  target_feature <- sub("^NOG__", "", get_clean_name(feature_string, keep_prefix = TRUE))
  matched_rows <- abundance_data %>% filter(eggNOG == target_feature)

  if (nrow(matched_rows) > 0) return(colSums(matched_rows[, sample_cols, drop = FALSE]))
  rep(0, length(sample_cols))
}

get_lefse_lda_for_feature <- function(feature_string, abundance_data, group1_name, group2_name, nog_level, sample_metadata, sample_cols) {
  cache_key <- paste(nog_level, group1_name, group2_name, sep = "_")
  ...
  if (!(cache_key %in% names(LEFSE_CACHE))) {
    ...
    agg_abundance$Feature_ID <- paste0("NOG_", sprintf("%05d", seq_len(nrow(agg_abundance))))
    ...
    # Scale relative abundance to pseudo-count scale to avoid -Inf LDA in microeco
    otu_table_comp <- otu_table_comp * 1e6

    tax_table_comp <- data.frame(
      Kingdom = rep(nog_level, nrow(agg_abundance)),
      Phylum = rep(nog_level, nrow(agg_abundance)),
    ...
    tryCatch({
      ...
      if (!is.null(comp_lefse$res_diff) && nrow(comp_lefse$res_diff) > 0) {
        comp_lefse$res_diff$Taxon_Name <- sapply(comp_lefse$res_diff$Taxa, function(x) {
          if (grepl("^NOG_", x) && x %in% names(feature_names_map)) return(feature_names_map[[x]])
          parts <- strsplit(x, "\\|")[[1]]
...
check_callback_trend <- function(feature_string, abundance_data, nog_level, sample_metadata, sample_cols) {
  ...
  lda_model_control <- get_lefse_lda_for_feature(feature_string, abundance_data, "Model", "Control", nog_level, sample_metadata, sample_cols)
  lda_hqqd_model <- get_lefse_lda_for_feature(feature_string, abundance_data, "HQQD", "Model", nog_level, sample_metadata, sample_cols)
...
for (level_info in nog_levels) {
  nog_level <- level_info$name
  level_column <- level_info$column

  cat("\n", rep("=", 70), "\n", sep = "")
  cat("=== Processing eggNOG", nog_level, "===\n")
  cat(rep("=", 70), "\n\n", sep = "")
  ...
  cat("Total eggNOG features at", nog_level, ":", length(unique_features), "\n")

  level_results <- data.frame(Taxa = character(), stringsAsFactors = FALSE)

  for (feature in unique_features) {
    feature_string <- paste0("NOG__", feature)
  ...
  for (i in seq_len(nrow(level_results))) {
    callback_results[[i]] <- check_callback_trend(level_results$Taxa[i], abundance_data, nog_level, sample_metadata, sample_cols)
    if (i %% 10 == 0) cat(".")
  }
  cat("\n")
  ...
  p_left <- ggplot(plot_data_combined, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
    ...
    labs(
      ...
      y = NULL,
      title = paste0(nrow(top_features), " Differential eggNOG Features at ", nog_level),
  ...
  output_file_pdf <- file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, ".pdf"))
  output_file_png <- file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, ".png"))
  ...
  # Additional output: keep only top features in image (same plotting details)
  top_features_top10 <- top_features %>% slice_head(n = TOP_N)

  if (nrow(top_features_top10) > 0) {
    plot_data_list_top10 <- vector("list", nrow(top_features_top10))
    for (i in seq_len(nrow(top_features_top10))) {
      feature_row <- top_features_top10[i, ]
      feature_string <- feature_row$Taxa
      clean_name <- get_clean_name(feature_string)
      abundances <- match_feature_to_abundance(feature_string, abundance_data)
      ...
      feature_data <- feature_data %>% left_join(sample_metadata, by = "Sample")
      feature_data$Log_Abundance <- log10(feature_data$Abundance + PSEUDO_COUNT)
      plot_data_list_top10[[i]] <- feature_data
    }

    plot_data_combined_top10 <- bind_rows(plot_data_list_top10)
    feature_order_top10 <- top_features_top10 %>%
      arrange(desc(LDA_avg)) %>%
      mutate(Clean_Name = sapply(Taxa, function(x) get_clean_name(x)))
    plot_data_combined_top10$Taxon <- factor(plot_data_combined_top10$Taxon, levels = rev(feature_order_top10$Clean_Name))
    ...
    p_left_top10 <- ggplot(plot_data_combined_top10, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
      ...
      labs(
        x = expression(Log[10] ~ "Relative Abundance"),
        y = NULL,
        title = paste0(nrow(top_features_top10), " Differential eggNOG Features at ", nog_level),
        subtitle = "Filtered by HQQD-induced Reversal, Sorted by LDA"
      ) +
      theme_bw(base_family = "sans") +
      theme(
    ...
    output_file_pdf_top10 <- file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, "_top5.pdf"))
    output_file_png_top10 <- file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, "_top5.png"))
  ...
  nog_meta <- abundance_data %>%
    select(eggNOG, descrition, eggNOG_Class, Category) %>%
    distinct(eggNOG, .keep_all = TRUE)

  stat_summary <- stat_summary %>%
    left_join(nog_meta, by = c("Taxon" = "eggNOG"))

  final_cols <- c(
    "Taxon", "descrition", "eggNOG_Class", "Category", "Taxa_Full",
  ...
  write.csv(stat_summary,
    file.path(output_dir, paste0("statistical_results_eggNOG_", nog_level, ".csv")),
    row.names = FALSE
  )

  write.csv(stat_summary_fdr,
    file.path(output_dir, paste0("statistical_results_eggNOG_", nog_level, "_fdr.csv")),
    row.names = FALSE
  )
  ...
  if (nrow(stat_summary_fdr) > 0) {
    ...
    save_reverse_boxplot(
      plot_data_fdr,
      file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, "_fdr.pdf")),
      file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, "_fdr.png")),
    ...
    save_reverse_boxplot(
      plot_data_fdr_top10,
      file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, "_top5_fdr.pdf")),
      file.path(output_dir, paste0("LEfSe_boxplot_eggNOG_", nog_level, "_top5_fdr.png")),
      "Top 5 eggNOG NOGs",
      plot_width = 9
    )
  }
...
cat("\n", rep("=", 70), "\n", sep = "")
cat("All eggNOG boxplots generated successfully!\n")
cat("Output directory:", output_dir, "\n")
cat("\nGenerated files:\n")
cat("  - LEfSe_boxplot_eggNOG_NOG.pdf/png\n")
cat("  - LEfSe_boxplot_eggNOG_NOG_fdr.pdf/png\n")
cat("  - LEfSe_boxplot_eggNOG_NOG_top5_fdr.pdf/png\n")
cat("  - statistical_results_eggNOG_NOG.csv (Wilcoxon p-values: Model vs Control, HQQD vs Model, Model vs LVX)\n")
cat("  - statistical_results_eggNOG_NOG_fdr.csv (BH-corrected callback subset)\n")
cat(rep("=", 70), "\n\n", sep = "")
