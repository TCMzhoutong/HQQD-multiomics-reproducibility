# ============================================================================
# LEfSe Differential CARD ARO Boxplot with Statistical Annotation (HQQD vs Model Selection)
# Author: Generated for metagenomics analysis
# Date: 2026-02-24
# Description: Horizontal boxplot showing differential CARD ARO functions
...
abundance_file <- "CARD.ARO.relabundance.xlsx"
output_dir <- "10b_HQQD_CARD_ARO"
...
cat("CARD data loaded:", nrow(abundance_data), "features\n")

metadata_cols <- c("ARO_accession", "ARO_name", "ARO_description", "Resistance", "Resistance Mechanism")
sample_cols <- setdiff(colnames(abundance_data), metadata_cols)
...
get_clean_name <- function(feature_string, keep_prefix = FALSE) {
  ...
  sub("^ARO__", "", last_level)
}

match_feature_to_abundance <- function(feature_string, abundance_data) {
  target_feature <- sub("^ARO__", "", get_clean_name(feature_string, keep_prefix = TRUE))
  matched_rows <- abundance_data[abundance_data[["ARO_name"]] == target_feature, ]
...
get_lefse_lda_for_feature <- function(feature_string, abundance_data, group1_name, group2_name, aro_level, sample_metadata, sample_cols) {
  cache_key <- paste(aro_level, group1_name, group2_name, sep = "_")
  ...
  if (!(cache_key %in% names(LEFSE_CACHE))) {
    ...
    agg_abundance <- abundance_data %>%
      filter(!is.na(ARO_name) & ARO_name != "") %>%
      group_by(ARO_name) %>%
    ...
    agg_abundance$Feature_ID <- paste0("ARO_", sprintf("%05d", seq_len(nrow(agg_abundance))))
    ...
    tax_table_comp <- data.frame(
      Kingdom = rep(aro_level, nrow(agg_abundance)),
      Phylum = rep(aro_level, nrow(agg_abundance)),
    ...
    tryCatch({
      ...
      if (!is.null(comp_lefse$res_diff) && nrow(comp_lefse$res_diff) > 0) {
        comp_lefse$res_diff$Taxon_Name <- sapply(comp_lefse$res_diff$Taxa, function(x) {
          if (grepl("^ARO_", x) && x %in% names(feature_names_map)) return(feature_names_map[[x]])
          parts <- strsplit(x, "\\|")[[1]]
...
check_callback_trend <- function(feature_string, abundance_data, aro_level, sample_metadata, sample_cols) {
  ...
  lda_model_control <- get_lefse_lda_for_feature(feature_string, abundance_data, "Model", "Control", aro_level, sample_metadata, sample_cols)
  lda_hqqd_model <- get_lefse_lda_for_feature(feature_string, abundance_data, "HQQD", "Model", aro_level, sample_metadata, sample_cols)
...
for (level_info in aro_levels) {
  aro_level <- level_info$name
  level_column <- level_info$column

  cat("\n", rep("=", 70), "\n", sep = "")
  cat("=== Processing CARD", aro_level, "===\n")
  cat(rep("=", 70), "\n\n", sep = "")
  ...
  cat("Total CARD features at", aro_level, ":", length(unique_features), "\n")

  level_results <- data.frame(Taxa = character(), stringsAsFactors = FALSE)

  for (feature in unique_features) {
    feature_string <- paste0("ARO__", feature)
  ...
  for (i in seq_len(nrow(level_results))) {
    callback_results[[i]] <- check_callback_trend(level_results$Taxa[i], abundance_data, aro_level, sample_metadata, sample_cols)
    if (i %% 10 == 0) cat(".")
  }
  cat("\n")
  ...
  cat("CARD features with significant callback trend:", nrow(sig_results), "\n")
  cat("CARD features retained after BH correction:", sum(level_results$is_callback_fdr, na.rm = TRUE), "\n")
  ...
  p_left <- ggplot(plot_data_combined, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
    ...
    labs(
      ...
      y = NULL,
      title = paste0(nrow(top_features), " Differential CARD Features at ", aro_level),
  ...
  output_file_pdf <- file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, ".pdf"))
  output_file_png <- file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, ".png"))
  ...
  # Additional output: Top 10 horizontal boxplot (keep all plotting details unchanged)
  top_features_top10 <- top_features %>% slice_head(n = TOP_N)
  if (nrow(top_features_top10) > 0) {
    top10_taxa <- sapply(top_features_top10$Taxa, function(x) get_clean_name(x))
    plot_data_combined_top10 <- plot_data_combined %>%
      filter(as.character(Taxon) %in% top10_taxa)

    feature_order_top10 <- top_features_top10 %>%
      arrange(desc(LDA_avg)) %>%
      mutate(Clean_Name = sapply(Taxa, function(x) get_clean_name(x)))
    plot_data_combined_top10$Taxon <- factor(as.character(plot_data_combined_top10$Taxon), levels = rev(feature_order_top10$Clean_Name))

    n_features_top10 <- length(levels(plot_data_combined_top10$Taxon))
    if (n_features_top10 >= 2) {
      alternating_bg_top10 <- data.frame(
        ymin = seq(1.5, n_features_top10 - 0.5, by = 1),
        ymax = seq(2.5, n_features_top10 + 0.5, by = 1)
      )
      alternating_bg_top10 <- alternating_bg_top10[seq(1, nrow(alternating_bg_top10), by = 2), , drop = FALSE]
    } else {
      alternating_bg_top10 <- data.frame(ymin = numeric(0), ymax = numeric(0))
    }

    p_left_top10 <- ggplot(plot_data_combined_top10, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
      geom_rect(data = alternating_bg_top10, aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax),
        fill = "gray85", alpha = 0.5, inherit.aes = FALSE) +
      geom_boxplot(position = position_dodge(width = 0.7), outlier.shape = NA, width = 0.35, alpha = 0.3, linewidth = 0.5) +
      scale_color_manual(values = group_colors) +
      scale_fill_manual(values = group_colors) +
      ...
      labs(
        x = expression(Log[10] ~ "Relative Abundance"),
        y = NULL,
        title = paste0(nrow(top_features_top10), " Differential CARD Features at ", aro_level),
        subtitle = "Filtered by HQQD-induced Reversal, Sorted by LDA"
      ) +
      theme_bw(base_family = "sans") +
      theme(
    ...
    p_middle_top10 <- ggplot(plot_data_combined_top10, aes(x = 1, y = Taxon)) +
      geom_rect(data = alternating_bg_top10, aes(xmin = -Inf, xmax = Inf, ymin = ymin, ymax = ymax),
        fill = "gray85", alpha = 0.5, inherit.aes = FALSE) +
      scale_y_discrete(limits = levels(plot_data_combined_top10$Taxon)) +
      theme_void(base_family = "sans") +
      theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), plot.margin = margin(10, 10, 10, 5), panel.border = element_blank()) +
      geom_segment(aes(x = Inf, xend = Inf, y = -Inf, yend = Inf), color = "black", linewidth = 0.5, inherit.aes = FALSE) +
      geom_segment(aes(x = -Inf, xend = Inf, y = Inf, yend = Inf), color = "black", linewidth = 0.5, inherit.aes = FALSE) +
    ...
    plot_stat_results_top10 <- list()
    for (feature in levels(plot_data_combined_top10$Taxon)) {
      fd <- plot_data_combined_top10 %>% filter(Taxon == feature)
      mc_v   <- fd$Log_Abundance[fd$Group == "Model"]
      ctrl_v <- fd$Log_Abundance[fd$Group == "Control"]
      hqqd_v <- fd$Log_Abundance[fd$Group == "HQQD"]
      lvx_v  <- fd$Log_Abundance[fd$Group == "LVX"]
      plot_stat_results_top10[[feature]] <- list(
        p_Model_vs_Control = tryCatch(wilcox.test(mc_v, ctrl_v, exact = FALSE)$p.value, error = function(e) NA),
        p_HQQD_vs_Model    = tryCatch(wilcox.test(hqqd_v, mc_v, exact = FALSE)$p.value, error = function(e) NA),
        p_Model_vs_LVX     = tryCatch(wilcox.test(mc_v, lvx_v,  exact = FALSE)$p.value, error = function(e) NA)
      )
    ...
    comparisons_fixed_top10 <- list(
      list(group1 = "Model", group2 = "Control", p_key = "p_Model_vs_Control"),
      list(group1 = "HQQD",  group2 = "Model",   p_key = "p_HQQD_vs_Model"),
      list(group1 = "Model", group2 = "LVX",     p_key = "p_Model_vs_LVX")
    )
    group_positions_top10 <- c("HQQD" = 0, "LVX" = 1, "Model" = 2, "Control" = 3)
    dodge_width_top10 <- 0.7
    n_groups_top10    <- 4

    for (feature_name in levels(plot_data_combined_top10$Taxon)) {
      feature_idx <- which(levels(plot_data_combined_top10$Taxon) == feature_name)
      stat_res    <- plot_stat_results_top10[[feature_name]]
      current_x   <- 2

      for (comp in comparisons_fixed_top10) {
        p_val <- stat_res[[comp$p_key]]
        if (is.na(p_val) || p_val >= 0.05) {
          current_x <- current_x + 1.2
          next
        ...
        sig_label <- dplyr::case_when(p_val < 0.001 ~ "***", p_val < 0.01 ~ "**", TRUE ~ "*")
        g1_idx <- group_positions_top10[comp$group1]
        g2_idx <- group_positions_top10[comp$group2]
        y1 <- feature_idx + (g1_idx - (n_groups_top10 - 1) / 2) * dodge_width_top10 / n_groups_top10
        y2 <- feature_idx + (g2_idx - (n_groups_top10 - 1) / 2) * dodge_width_top10 / n_groups_top10
        star_spacing <- dplyr::case_when(sig_label == "***" ~ 2.6, sig_label == "**" ~ 2.0, TRUE ~ 1.2)

        p_middle_top10 <- p_middle_top10 +
          annotate("segment", x = current_x, xend = current_x, y = y1, yend = y2, color = "gray30", linewidth = 0.6) +
          annotate("text", x = current_x + 0.3, y = (y1 + y2) / 2, label = sig_label,
                   size = 3.5, fontface = "bold", color = "gray20", hjust = 0)
    ...
    p_legend_temp_top10 <- ggplot(plot_data_combined_top10, aes(x = Log_Abundance, y = Taxon, color = Group, fill = Group)) +
      geom_boxplot(alpha = 0.3) +
      scale_color_manual(values = group_colors, name = "Group") +
      scale_fill_manual(values = group_colors, name = "Group") +
      guides(color = guide_legend(reverse = TRUE, override.aes = list(fill = group_colors, alpha = 0.3)), fill = "none") +
    ...
    p_legend_top10 <- as_ggplot(get_legend(p_legend_temp_top10))
    p_combined_top10 <- p_left_top10 + p_middle_top10 + p_legend_top10 + plot_layout(widths = c(5, 1, 0.8))

    output_file_pdf_top10 <- file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, "_top10.pdf"))
    output_file_png_top10 <- file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, "_top10.png"))

    plot_height_top10 <- max(6, nrow(top_features_top10) * 0.55 + 2)
    plot_width_top10 <- 8.5

    ggsave(filename = output_file_pdf_top10, plot = p_combined_top10, width = plot_width_top10, height = plot_height_top10, units = "in", dpi = 300)
    ggsave(filename = output_file_png_top10, plot = p_combined_top10, width = plot_width_top10, height = plot_height_top10, units = "in", dpi = 300)
  }
  ...
  aro_meta <- abundance_data %>%
    select(ARO_name, ARO_accession, ARO_description, Resistance, `Resistance Mechanism`) %>%
    distinct(ARO_name, .keep_all = TRUE)

  stat_summary <- stat_summary %>%
    left_join(aro_meta, by = c("Taxon" = "ARO_name"))

  final_cols <- c(
    "Taxon", "ARO_accession", "ARO_description", "Resistance", "Resistance Mechanism", "Taxa_Full",
  ...
  write.csv(stat_summary,
    file.path(output_dir, paste0("statistical_results_CARD_", aro_level, ".csv")),
    row.names = FALSE
  )

  write.csv(stat_summary_fdr,
    file.path(output_dir, paste0("statistical_results_CARD_", aro_level, "_fdr.csv")),
    row.names = FALSE
  )
  ...
  if (nrow(stat_summary_fdr) > 0) {
    ...
    save_reverse_boxplot(
      plot_data_fdr,
      file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, "_fdr.pdf")),
      file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, "_fdr.png")),
      paste0(nrow(stat_summary_fdr), " FDR-corrected Differential CARD Features at ", aro_level),
      plot_width = plot_width
    )

    fdr_order_top10 <- head(fdr_order, TOP_N)
    plot_data_fdr_top10 <- plot_data_combined %>%
      filter(as.character(Taxon) %in% fdr_order_top10)
    plot_data_fdr_top10$Taxon <- factor(as.character(plot_data_fdr_top10$Taxon), levels = rev(fdr_order_top10))
    save_reverse_boxplot(
      plot_data_fdr_top10,
      file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, "_top10_fdr.pdf")),
      file.path(output_dir, paste0("LEfSe_boxplot_CARD_", aro_level, "_top10_fdr.png")),
      paste0(length(fdr_order_top10), " FDR-corrected Differential CARD Features at ", aro_level),
      plot_width = plot_width
    )
  }
}
...
cat("\n", rep("=", 70), "\n", sep = "")
cat("All CARD boxplots generated successfully!\n")
cat("Output directory:", output_dir, "\n")
cat("\nGenerated files:\n")
cat("  - LEfSe_boxplot_CARD_ARO.pdf/png\n")
cat("  - LEfSe_boxplot_CARD_ARO_fdr.pdf/png\n")
cat("  - LEfSe_boxplot_CARD_ARO_top10_fdr.pdf/png\n")
cat("  - statistical_results_CARD_ARO.csv (Wilcoxon p-values: Model vs Control, HQQD vs Model, Model vs LVX)\n")
cat("  - statistical_results_CARD_ARO_fdr.csv (BH-corrected callback subset)\n")
cat(rep("=", 70), "\n\n", sep = "")
