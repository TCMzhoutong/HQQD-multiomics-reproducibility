# ============================================================================
# Beta Diversity NMDS Analysis (Multi-Taxonomic Levels)
...
#              with PERMANOVA statistical testing and academic-standard plotting
# ============================================================================
...
taxonomic_levels <- list(
  list(file = "taxonomy.phylum.relabundance.xlsx", 
       level = "Phylum", 
       output_dir = "03_beta_NMDS_Phylum"),
  list(file = "taxonomy.genus.relabundance.xlsx", 
       level = "Genus", 
       output_dir = "03_beta_NMDS_Genus"),
  list(file = "taxonomy.species.relabundance.xlsx", 
       level = "Species", 
       output_dir = "03_beta_NMDS_Species")
)
...
plot_nmds <- function(nmds_coords, metadata, method_name, taxonomic_level, permanova_result, stress_value) {
  ...
  # Get column names for NMDS axes (they might be MDS1/MDS2 or NMDS1/NMDS2)
...
for (tax_level in taxonomic_levels) {
  cat("\n============================================================================\n")
  cat(paste0("=== Analyzing ", tax_level$level, " Level (NMDS) ===\n"))
  cat("============================================================================\n")
  ...
  abundance_matrix <- abundance_data[, -1] %>% as.data.frame() %>% t()
  ...
  write.csv(permanova_results, file.path(output_dir, "PERMANOVA_and_Stress.csv"), row.names = FALSE)
  ...
  ggsave(file.path(output_dir, "NMDS_Binary_Jaccard.png"), plot = p_jaccard, width = 6, height = 5, dpi = 300)
  ggsave(file.path(output_dir, "NMDS_Binary_Jaccard.pdf"), plot = p_jaccard, width = 6, height = 5, device = cairo_pdf)
  ggsave(file.path(output_dir, "NMDS_Bray_Curtis.png"), plot = p_bray, width = 6, height = 5, dpi = 300)
  ggsave(file.path(output_dir, "NMDS_Bray_Curtis.pdf"), plot = p_bray, width = 6, height = 5, device = cairo_pdf)
  cat("Individual NMDS plots saved!\n")
  
  # Save NMDS coordinates
  write.csv(nmds_jaccard_coords, file.path(output_dir, "NMDS_Binary_Jaccard_coordinates.csv"), row.names = FALSE)
  write.csv(nmds_bray_coords, file.path(output_dir, "NMDS_Bray_Curtis_coordinates.csv"), row.names = FALSE)
  ...
  cat("\n============================================================================\n")
  cat(paste0("=== ", tax_level$level, " Level (NMDS) Analysis Complete ===\n"))
  cat("============================================================================\n\n")
}

cat("\n============================================================================\n")
cat("=== All Taxonomic Level NMDS Analyses Complete ===\n")
cat("============================================================================\n\n")

cat("=== Key Points ===\n")
cat("1. NMDS uses relative abundance data (correct for beta diversity)\n")
cat("2. Stress values are displayed on plots (bottom-right corner)\n")
cat("3. PERMANOVA tests group differences (top-left corner)\n")
cat("4. Stress interpretation:\n")
...
cat("\n5. Both metrics (Jaccard and Bray-Curtis) provide complementary views:\n")
cat("   • Binary Jaccard: Presence/absence (unweighted)\n")
cat("   • Bray-Curtis: Abundance-weighted\n")
cat("\n=== Methods Text Suggestion ===\n")
cat('  "Beta diversity was visualized using Non-metric Multidimensional\n')
cat('   Scaling (NMDS) based on Binary Jaccard and Bray-Curtis distance\n')
cat('   matrices at three taxonomic levels (phylum, genus, and species).\n')
cat('   PERMANOVA was used to test for significant differences among\n')
cat('   treatment groups (999 permutations). Stress values < 0.2 indicate\n')
cat('   acceptable ordination quality. All analyses were performed in R\n')
cat('   using the vegan package."\n')
cat("\n============================================================================\n\n")
